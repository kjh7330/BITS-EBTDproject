...
public class AdminCompanyMM {
	...
	public ModelAndView setNewBusRouteReject(String ap_b_no, String ap_reject) {
		...
		String view = null;
		List<ApplyBusHistory> cList = null;
		cDao.setApplyBusHistoryChangeReject(ap_b_no, ap_reject);
		cList = cDao.getNewBusRouteList();
		System.out.println(cList);//
		for(int i=0; i<cList.size();i++) {
			if(cList.get(i).getAp_b_no() == ap_b_no && cList.get(i).getAp_state() != 2) {
				view="admin/mainForm";
				System.out.println("신규버스 반려 실패! 쿠구구궁");
			}else {
				mav.addObject("cList", cList);
				view="admin/company/newBusRouteListForm";
			}
	...
	public ModelAndView getCompanyDetail(String c_username) {
		...
		List<Company> bdCountList = null; // 총버스수, 기사수
		...
		System.out.println(vrCountList);
		System.out.println(bdCountList);
		System.out.println(lrCountList);
	...
	public ModelAndView getCompanyQuestionList(String c_username) {
		...
		vrCountList = cDao.getVocRecommendCount(c_username);
		vocList = cDao.getCompanyQuestionList();
