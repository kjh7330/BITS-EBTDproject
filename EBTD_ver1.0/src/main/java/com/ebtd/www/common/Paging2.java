...
public class Paging2 {
	...
	private HttpSession sss;

	public Paging2(int maxNum, int pageNum, int listCount, int pageCount, String stopList, HttpSession sss) {
		this.maxNum = maxNum;
	...
	private String makeHtml(int currentGroup, int totalPage, String stopList) {
		...
		if (start != 1) {
			sb.append("<a href='"+stopList+"?pageNum=" + (start -1) + ( (sss.getAttribute("search") == null) ? "":"&search="+ sss.getAttribute("search") ) + "'>");
			sb.append("[이전]");
			sb.append("</a>");
		}
		...
		for (int i = start; i <= end; i++) {
			if (pageNum != i) { //현재 페이지가 아닌 경우 링크처리
				sb.append("<a href='"+stopList+"?pageNum=" + i + ( (sss.getAttribute("search") == null) ? "":"&search="+ sss.getAttribute("search") ) + "'>");
				sb.append(" [ ");
				sb.append(i);
				sb.append(" ] ");
				sb.append("</a>");
		...
		if (end != totalPage) {
			sb.append("<a href='"+stopList+"?pageNum=" + (end + 1) + ( (sss.getAttribute("search") == null) ? "":"&search="+ sss.getAttribute("search") ) + "'>");
			sb.append("[다음]");
			sb.append("</a>");
		}
		return sb.toString();
