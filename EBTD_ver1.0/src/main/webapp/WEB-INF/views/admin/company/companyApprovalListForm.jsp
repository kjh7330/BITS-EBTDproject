...
<script type="text/javascript">
	...
	for(i in baList){
		...
		all += '<td><a href=/admin/company/getCompanyApprovalDetail?ap_b_no='+baList[i]["ap_b_no"]+' style="text-decoration: none;">';
	...
	function fnCtg(){
		...
		for(i in baList){//2021-08-11
			...
			if( ( cur_date >= ba_date && ba_date >= period ) || val2 == 1 ){
				...
				c_name += '<td><a href=/admin/company/getCompanyApprovalDetail?ap_b_no='+baList[i]["ap_b_no"]+' style="text-decoration: none;">';
				c_name += baList[i]["ap_b_no"] +'번</td>';
				c_name += '<td>' + baList[i]["ap_date"] + '</td>';
					let state = baList[i]["ap_state"];
				 	if	   (state == 1){ c_name += '<td>등록 승인</td>'; }
				 	else if(state == 4){ c_name += '<td>변경 승인</td>'; 	}
				c_name += '</tr>';
			}
		}
